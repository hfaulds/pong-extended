...
use systems::{BounceSystem, CheatSystem, MoveBallsSystem, PaddleSystem, WinnerSystem};
...
impl<'a, 'b> ECSBundle<'a, 'b> for PongBundle {
    fn build(self, _: &mut World, builder: &mut DispatcherBuilder<'a, 'b>) -> Result<()> {
        ...
        builder.add(CheatSystem::new(), "cheat_system", &[]);
        builder.add(
            BounceSystem,
            "collision_system",
        ...
        builder.add(
            WinnerSystem,
            "winner_system",
            &["paddle_system", "ball_system", "cheat_system"],
        );
        Ok(())
    }
}
